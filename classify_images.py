...
def classify_images(images_dir, results_dic, model):
    ...
    for (filename, label) in results_dic.items():
        ...
        formatted_classifier_labels = list(map(
            lambda x: x.lower().strip(), classifier_label.split(',')))
        is_a_match = 1 if (label[0] in formatted_classifier_labels) else 0
        results_dic[filename].append(classifier_label)
        results_dic[filename].append(is_a_match)
